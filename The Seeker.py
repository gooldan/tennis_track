...
while True:
    ...
    for elem in res2:
        x=elem.pt[0]
        y=elem.pt[1]
        cv2.circle(diff2, (int(x), int(y)), 5, (0, 150, 0), 2) 
        #cv2.putText(diff2, 'BALL', (int(x)+10, int(y)+10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 250, 0), 1)
    for elem in res1:
        x=elem.pt[0]
        y=elem.pt[1]
    ...
    #    move=True
    #    for thing in res2:
    #        if dist(elem.pt[0], elem.pt[1], thing.pt[0], thing.pt[1]) < 10:
    #            cv2.line(diff1, (int(elem.pt[0]), int(elem.pt[1])), (int(thing.pt[0]), int(thing.pt[1])), (255, 0, 0), 2)
    #            move=False
    #            break
    #    if move==True:
    ...
    if len(res1)>0:
        ...
        distanse=200
        for elem in res1:
            if (dist(elem.pt[0], elem.pt[1], xnoga, ynoga) > distanse):
                distanse =dist(elem.pt[0], elem.pt[1], xnoga, ynoga)
                cv2.line(frame1, (int(elem.pt[0]), int(elem.pt[1])), (int(xnoga), int(ynoga)), (255, 0, 0), 2)
                cv2.putText(frame1, str(distanse), (int(elem.pt[0])+10, int(elem.pt[1])+10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (33, 255, 237), 1)
                ball.clear()
                ball.append(elem)
